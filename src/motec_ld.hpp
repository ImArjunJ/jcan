...
// Binary format reverse-engineered; reference: github.com/gotzl/ldparser
...
struct ld_channel
{
    ...
    int16_t scale{};
    int16_t dec_places{};
    std::vector<double> samples; // physical values after calibration
};
...
struct ld_file
{
    ...
    [[nodiscard]] double duration_seconds() const
    {
        ...
        for (const auto& ch : channels)
        {
            ...
            {
                double dur = static_cast<double>(ch.samples.size()) / static_cast<double>(ch.freq_hz);
...
[[nodiscard]] inline T read_le(const uint8_t* buf, std::size_t offset)
{
    ...
    return val; // x86/ARM LE: memcpy is sufficient
}

[[nodiscard]] inline std::string read_string(const uint8_t* buf, std::size_t offset, std::size_t max_len)
{
...
// header layout (total 1762 bytes, matching Python struct '<I4x II 20x I 24x HHH I 8s H H I 4x 16s 16x 16s 16x 64s 64s 64x 64s 64x 1024x I 66x 64s 126x')
// offsets computed from cumulative sizes:
constexpr std::size_t k_head_size = 1762;

constexpr std::size_t off_magic = 0;          // u32 (must be 0x40)
// 4 bytes padding
constexpr std::size_t off_meta_ptr = 8;       // u32
constexpr std::size_t off_data_ptr = 12;      // u32
// 20 bytes padding
constexpr std::size_t off_event_ptr = 36;     // u32
// 24 bytes padding
// HHH (3 unknown u16) at 64..70
constexpr std::size_t off_device_serial = 70; // u32
constexpr std::size_t off_device_type = 74;   // 8s
constexpr std::size_t off_device_ver = 82;    // u16
// u16 unknown at 84
constexpr std::size_t off_num_channs = 86;    // u32
// 4 bytes padding
constexpr std::size_t off_date = 94;          // 16s
// 16 padding
constexpr std::size_t off_time = 126;         // 16s
// 16 padding
constexpr std::size_t off_driver = 158;       // 64s
constexpr std::size_t off_vehicleid = 222;    // 64s
// 64 padding
constexpr std::size_t off_venue = 350;        // 64s
// 64 padding
// 1024 padding
// pro logging flag u32 at 1502
// 66 padding
constexpr std::size_t off_short_comment = 1572; // 64s
// 126 padding → total 1762

// channel metadata layout (124 bytes each)
constexpr std::size_t k_chan_size = 124;

constexpr std::size_t ch_off_prev = 0;        // u32
constexpr std::size_t ch_off_next = 4;        // u32
constexpr std::size_t ch_off_data_ptr = 8;    // u32
constexpr std::size_t ch_off_data_len = 12;   // u32 (sample count, not bytes)
constexpr std::size_t ch_off_counter = 16;    // u16
constexpr std::size_t ch_off_dtype_a = 18;    // u16
constexpr std::size_t ch_off_dtype = 20;      // u16
constexpr std::size_t ch_off_freq = 22;       // u16
constexpr std::size_t ch_off_shift = 24;      // i16
constexpr std::size_t ch_off_mul = 26;        // i16
constexpr std::size_t ch_off_scale = 28;      // i16
constexpr std::size_t ch_off_dec = 30;        // i16
constexpr std::size_t ch_off_name = 32;       // 32 bytes
constexpr std::size_t ch_off_short = 64;      // 8 bytes
constexpr std::size_t ch_off_unit = 72;       // 12 bytes
// 40 bytes padding → total 124

// event layout (1154 bytes)
constexpr std::size_t k_event_size = 1154;
constexpr std::size_t ev_off_name = 0;        // 64s
constexpr std::size_t ev_off_session = 64;    // 64s
constexpr std::size_t ev_off_comment = 128;   // 1024s
constexpr std::size_t ev_off_venue_ptr = 1152; // u16

// venue layout (1100 bytes)
constexpr std::size_t k_venue_size = 1100;
constexpr std::size_t vn_off_name = 0;        // 64s
// 1034 padding
constexpr std::size_t vn_off_vehicle_ptr = 1098; // u16

// vehicle layout (260 bytes)
constexpr std::size_t k_vehicle_size = 260;
constexpr std::size_t vh_off_id = 0;          // 64s
// 128 padding
constexpr std::size_t vh_off_weight = 192;    // u32
constexpr std::size_t vh_off_type = 196;      // 32s
constexpr std::size_t vh_off_comment = 228;   // 32s

[[nodiscard]] inline bool in_bounds(std::size_t buf_size, std::size_t offset, std::size_t need)
{
    return offset <= buf_size && (buf_size - offset) >= need;
}

// determine element size from dtype_a and dtype fields
// returns 0 on unknown type
[[nodiscard]] inline std::size_t element_size(uint16_t dtype_a, uint16_t dtype)
{
    // dtype field: 2 → 2-byte element, 4 → 4-byte element
    if (dtype == 2) return 2;
    if (dtype == 4) return 4;
    return 0;
}
...
// decode a single raw sample to double based on type
[[nodiscard]] inline double decode_sample(const uint8_t* buf, std::size_t offset,
                                          uint16_t dtype_a, uint16_t dtype)
{
    if (dtype_a == 0x07)
    {
        // float types
    ...
    {
        // integer types (dtype_a in {0x00, 0x03, 0x05})
        if (dtype == 4)
        {
            int32_t v;
            std::memcpy(&v, buf + offset, 4);
...
[[nodiscard]] inline std::expected<ld_file, std::string> load_ld(const std::filesystem::path& path)
{
    ...
    // parse event → venue → vehicle chain
    if (event_ptr > 0 && in_bounds(file_size, event_ptr, k_event_size))
    {
        ...
        if (venue_ptr > 0 && in_bounds(file_size, venue_ptr, k_venue_size))
        {
            ...
            uint16_t vehicle_ptr = read_le<uint16_t>(d, venue_ptr + vn_off_vehicle_ptr);
            if (vehicle_ptr > 0 && in_bounds(file_size, vehicle_ptr, k_vehicle_size))
            {
    ...
    // parse channel metadata (linked list)
    std::unordered_set<std::string> seen_names;
    uint32_t chan_ptr = meta_ptr;
    std::size_t max_channels = 1024; // safety limit

    while (chan_ptr > 0 && max_channels-- > 0)
    {
        if (!in_bounds(file_size, chan_ptr, k_chan_size))
            break;
        ...
        if (elem_sz > 0 && data_len > 0 && ch.freq_hz > 0)
        {
            ...
            if (in_bounds(file_size, data_ptr, byte_count))
            {
                ...
                double mul = (ch.multiplier != 0) ? static_cast<double>(ch.multiplier) : 1.0;
                double dec_factor = std::pow(10.0, static_cast<double>(-ch.dec_places));
                double shift = static_cast<double>(ch.shift);

                for (uint32_t i = 0; i < data_len; ++i)
                {
                    double raw = decode_sample(d, data_ptr + static_cast<std::size_t>(i) * elem_sz,
                                               dtype_a, dtype);
