...
using fn_canGetChannelData = canStatus(__stdcall*)(int channel, int item,
                                                   void* buffer,
                                                   size_t bufsize);
using fn_canOpenChannel = canHandle(__stdcall*)(int channel, int flags);
using fn_canClose = canStatus(__stdcall*)(canHandle hnd);
using fn_canSetBusParams = canStatus(__stdcall*)(
    canHandle hnd, long freq, unsigned int tseg1, unsigned int tseg2,
...
using fn_canWrite = canStatus(__stdcall*)(canHandle hnd, long id, void* msg,
                                          unsigned int dlc, unsigned int flag);
using fn_canRead = canStatus(__stdcall*)(canHandle hnd, long* id, void* msg,
                                         unsigned int* dlc, unsigned int* flag,
                                         unsigned long* time);
using fn_canReadWait = canStatus(__stdcall*)(canHandle hnd, long* id, void* msg,
                                             unsigned int* dlc,
                                             unsigned int* flag,
                                             unsigned long* time,
                                             unsigned long timeout);
using fn_canSetBusOutputControl =
    canStatus(__stdcall*)(canHandle hnd, unsigned int drivertype);
using fn_canGetErrorText = canStatus(__stdcall*)(canStatus err, char* buf,
                                                 unsigned int bufsiz);
...
struct canlib_api {
  ...
  bool load() {
    ...
    canInitializeLibrary =
        reinterpret_cast<fn_canInitializeLibrary>(get("canInitializeLibrary"));
    canUnloadLibrary =
        reinterpret_cast<fn_canUnloadLibrary>(get("canUnloadLibrary"));
    canGetNumberOfChannels = reinterpret_cast<fn_canGetNumberOfChannels>(
        get("canGetNumberOfChannels"));
    canGetChannelData =
        reinterpret_cast<fn_canGetChannelData>(get("canGetChannelData"));
    canOpenChannel = reinterpret_cast<fn_canOpenChannel>(get("canOpenChannel"));
    canClose = reinterpret_cast<fn_canClose>(get("canClose"));
    canSetBusParams =
        reinterpret_cast<fn_canSetBusParams>(get("canSetBusParams"));
    ...
    canSetBusOutputControl = reinterpret_cast<fn_canSetBusOutputControl>(
        get("canSetBusOutputControl"));
    canGetErrorText =
        reinterpret_cast<fn_canGetErrorText>(get("canGetErrorText"));

    if (!canInitializeLibrary || !canOpenChannel || !canClose ||
        !canSetBusParams || !canBusOn || !canBusOff || !canWrite ||
        !canReadWait || !canSetBusOutputControl) {
      FreeLibrary(dll);
      dll = nullptr;
      return false;
    }
...
inline std::vector<channel_info> enumerate_channels() {
  ...
  if (!ensure_loaded()) {
    if (dbg) std::fprintf(stderr, "[canlib] canlib32.dll not loaded\n");
    return out;
  }
  ...
  if (!a.canGetNumberOfChannels || a.canGetNumberOfChannels(&count) != canOK) {
    if (dbg) std::fprintf(stderr, "[canlib] canGetNumberOfChannels failed\n");
    return out;
  }
  ...
  for (int i = 0; i < count; ++i) {
    ...
    if (test_hnd < 0) {
      if (dbg)
        std::fprintf(
            stderr,
            "[canlib] ch %d: canOpenChannel probe failed (%d), skipping\n", i,
            test_hnd);
      continue;
    }
    a.canClose(test_hnd);
    ...
    char name_buf[256]{};
    if (a.canGetChannelData &&
        a.canGetChannelData(i, canCHANNELDATA_DEVDESCR_ASCII, name_buf,
                            sizeof(name_buf)) == canOK) {
      ci.device_name = name_buf;
    }

    char chan_name[256]{};
    if (a.canGetChannelData &&
        a.canGetChannelData(i, canCHANNELDATA_CHANNEL_NAME, chan_name,
                            sizeof(chan_name)) == canOK) {
      ci.name = chan_name;
    }

    int ch_on_card = 0;
    if (a.canGetChannelData &&
        a.canGetChannelData(i, canCHANNELDATA_CHAN_NO_ON_CARD, &ch_on_card,
                            sizeof(ch_on_card)) == canOK) {
      ci.channel_on_card = ch_on_card;
    }

    if (ci.device_name.empty()) ci.device_name = "Kvaser";
...
struct kvaser_canlib {
  ...
  [[nodiscard]] result<> close() {
    ...
    if (debug()) std::fprintf(stderr, "[kvaser-canlib] closed\n");
    return {};
  }
  ...
  [[nodiscard]] result<> send(const can_frame& frame) {
    ...
    if (frame.rtr) flags |= canlib::canMSG_RTR;

    uint8_t buf[64]{};
    std::memcpy(buf, frame.data.data(), payload_len);

    auto stat =
        a.canWrite(hnd_, static_cast<long>(frame.id), buf, payload_len, flags);
    if (stat != canlib::canOK) {
      if (debug())
  ...
  [[nodiscard]] result<std::optional<can_frame>> recv(
    ...
    auto stat =
        a.canReadWait(hnd_, &id, buf, &dlc, &flags, &timestamp, timeout_ms);
  ...
  [[nodiscard]] result<std::vector<can_frame>> recv_many(
...

    {
      ...
      if (r->has_value()) frames.push_back(r->value());
    }
    ...
    for (int i = 0; i < 1000; ++i) {
      ...
      auto stat =
          a.canRead
              ? a.canRead(hnd_, &id, buf, &dlc, &flags, &timestamp)
              : a.canReadWait(hnd_, &id, buf, &dlc, &flags, &timestamp, 0);

      if (stat == canlib::canERR_NOMSG) break;
      if (stat != canlib::canOK) break;
