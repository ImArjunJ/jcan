...
static void glfw_drop_callback([[maybe_unused]] GLFWwindow* window, int count, const char** paths)
{
    if (!g_drop_state || count <= 0)
        return;
    if (!g_drop_state->log_mode)
        return;
        ...
        if (path.size() >= 4)
        {
            ...
            if (ext == ".dbc")
            {
                if (g_drop_state->dbc.load(path))
                {
                    ...
                    g_drop_state->status_text = std::format("DBC: {} msgs", g_drop_state->dbc.message_ids().size());
                }
                else
                {
...
static void setup_default_layout(ImGuiID dockspace_id)
{
    ...
    ImGui::DockBuilderSplitNode(bottom, ImGuiDir_Left, 0.60f, &bot_left, &bot_right);
...
int main()
{
    try
    {
        ...
        GLFWwindow* window = glfwCreateWindow(settings.window_width, settings.window_height, "jcan - CAN Bus Tool", nullptr, nullptr);
        if (!window)
        {
            glfwTerminate();
            return 1;
        ...
        ImFont* mono_font = nullptr;
        {
            ...
            if (!mono_font_path.empty())
                mono_font = io.Fonts->AddFontFromFileTTF(mono_font_path.c_str(), font_size);
            io.Fonts->Build();
        }
        ...
        while (!glfwWindowShouldClose(window))
        {
            ...
            if (ImGui::BeginMainMenuBar())
            {
                if (ImGui::BeginMenu("File"))
                {
                    if (state.log_mode && !state.imported_frames.empty())
                    {
                        ...
                        {
                            if (ImGui::MenuItem("Load DBC...", "Ctrl+O", false, !file_dialog.busy()))
                            {
                            ...
                            {
                                ImGui::SameLine();
                                ImGui::TextDisabled("(%s)", state.dbc.filenames().front().c_str());
                        ...
                        else
                        {
                            if (ImGui::BeginMenu("Log DBC"))
                            {
                                for (uint8_t ch : state.log_channels)
                                {
                                    ...
                                    if (has)
                                    {
                                        ImGui::TextDisabled("%s: %s", label.c_str(), it->second.filenames().front().c_str());
                                        ImGui::SameLine();
                                    ...
                                    else
                                    {
                                        ...
                                        if (ImGui::MenuItem(btn.c_str(), nullptr, false, !file_dialog.busy()))
                                        {
                                            pending_dbc_channel = ch;
                                            file_dialog.open_file({{"DBC Files", "dbc"}});
                                            pending_dialog = dialog_id::open_dbc;
                    ...
                    {
                        auto log_label = std::format("Logging to: {} ({} frames)", state.logger.filename(), state.logger.frame_count());
                        ...
                        if (ImGui::MenuItem("Export Log...", "Ctrl+E", false, !file_dialog.busy()))
                        {
                            file_dialog.save_file({{"CSV Log", "csv"}, {"Vector ASC", "asc"}}, "export.csv");
                    ...
                    {
                        if (ImGui::MenuItem("Replay Log...", nullptr, false, !file_dialog.busy()))
                        {
                    ...
                    else
                    {
                        ...
                        auto replay_label = std::format("Stop Replay ({:.0f}%)", state.replay_progress.load() * 100.f);
                ...
                if (ImGui::BeginMenu("View"))
                {
                    ...
                    {
                        static constexpr float presets[] = {0.5f, 0.75f, 1.0f, 1.25f, 1.5f, 2.0f};
                        for (float p : presets)
                        {
                ...
                {
                    auto conn_label = state.connected ? std::format("Connected ({} adapter{})", state.adapter_slots.size(), state.adapter_slots.size() > 1 ? "s" : "")
                ...
                if (state.exporting.load())
                    full_status += std::format("[EXPORT:{:.0f}%] ", state.export_progress.load() * 100.f);
                if (state.replaying.load())
                {
                    bool paused = state.replay_paused.load();
                    float speed = state.replay_speed.load();
                    full_status +=
                        std::format("[REPLAY:{:.0f}%{}{}] ", state.replay_progress.load() * 100.f, paused ? " PAUSED" : "", speed != 1.0f ? std::format(" {:.2g}x", speed) : "");
                }
                if (state.dbc.loaded())
                {
                    auto dbc_names = state.dbc.filenames();
                    for (const auto& dn : dbc_names) full_status += std::format("[{}] ", dn);
                }
                ...
                if (state.connected)
                    ImGui::TextColored(ImVec4(0.3f, 1.0f, 0.3f, 1.0f), "%s", full_status.c_str());
                else
                    ImGui::TextColored(ImVec4(1.0f, 0.4f, 0.4f, 1.0f), "%s", full_status.c_str());
            ...
            if (io.KeyCtrl && ImGui::IsKeyPressed(ImGuiKey_Q))
                glfwSetWindowShouldClose(window, GLFW_TRUE);
            if (io.KeyCtrl && ImGui::IsKeyPressed(ImGuiKey_R) && state.logger.recording())
            {
                state.logger.stop();
                state.auto_start_session_log();
            }
            if (io.KeyCtrl && ImGui::IsKeyPressed(ImGuiKey_E) && !file_dialog.busy() && state.logger.recording() && !state.exporting.load())
            {
                file_dialog.save_file({{"CSV Log", "csv"}, {"Vector ASC", "asc"}}, "export.csv");
                pending_dialog = dialog_id::export_log;
            }
            if (state.log_mode && io.KeyCtrl && ImGui::IsKeyPressed(ImGuiKey_O) && !file_dialog.busy())
            {
                pending_dbc_channel = 0xff;
                file_dialog.open_file({{"DBC Files", "dbc"}});
                pending_dialog = dialog_id::open_dbc;
            ...
            if (auto result = file_dialog.poll())
            {
                ...
                case dialog_id::open_dbc:
                    if (*result)
                    {
                        if (pending_dbc_channel == 0xff)
                        {
                            ...
                            {
                                state.redecode_log();
                                state.status_text = std::format("DBC: {} msgs", state.dbc.message_ids().size());
                            }
                            else
                            {
                        ...
                        else
                        {
                            ...
                            {
                                state.redecode_log();
                                state.status_text = std::format("Ch {} DBC: {} msgs", static_cast<int>(pending_dbc_channel), eng.message_ids().size());
                            }
                            else
                            {
                                state.status_text = "DBC load failed!";
                ...
                case dialog_id::export_log:
                    if (*result)
                    {
                        ...
                        state.status_text = std::format("Exporting {} frames...", state.logger.frame_count());
                    }
                    break;
            ...
            if (state.show_signals)
                jcan::widgets::draw_signals(state);
            if (state.show_plotter)
            {
                jcan::widgets::draw_plotter(state, plotter);
            }
            if (state.show_transmitter)
                jcan::widgets::draw_transmitter(state);
            if (state.show_statistics)
                jcan::widgets::draw_statistics(state);
