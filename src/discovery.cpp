...
#if !defined(_WIN32) && (defined(JCAN_HAS_VECTOR) || defined(JCAN_HAS_KVASER))
#include <libusb.h>
#endif

#if defined(_WIN32) && defined(JCAN_HAS_VECTOR)
#include "hardware_vector_xlapi.hpp"
#endif

#if defined(JCAN_HAS_KVASER) && !defined(_WIN32)
        ...
        if (sp_list_ports(&port_list) == SP_OK && port_list)
        {
            for (int i = 0; port_list[i]; ++i)
            {
                ...
                if (auto* known = find_known_serial(vid, pid); known)
                {
                    ...
                    d.friendly_name = std::format("{} ({})", desc ? desc : d.port, known->label);
            ...
            for (const auto& entry : fs::directory_iterator(usb_devices))
            {
                ...
                if (vid == 0x1248)
                {
                    if (auto* vdev = find_known_vector(pid); vdev)
                    {
                        ...
                        for (int ch = 0; ch < vdev->num_channels; ++ch)
                        {
                            ...
                            d.friendly_name = std::format("Vector {} CH{} ({:04X}:{:04X})", product, ch + 1, vid, pid);
                            out.push_back(std::move(d));
                        }
                        continue;
                    }
                ...
                if (vid == 0x0BFD)
                {
                    ...
                    for (uint8_t ch = 0; ch < channels; ++ch)
                    {
                        ...
                        d.friendly_name = std::format("Kvaser {} CH{} ({:04X}:{:04X})", product, ch + 1, vid, pid);
                        out.push_back(std::move(d));
                    }
                    continue;
                }
...
#if defined(_WIN32) && defined(JCAN_HAS_VECTOR)
        {
            std::fprintf(stderr, "[vector] scanning for Vector devices via XL Driver Library...\n");
            ...
            if (xl.ensure_driver_open())
            {
                ...
                {
                    std::fprintf(stderr, "[vector] vxlapi: dllVersion=0x%X channelCount=%u\n", config.dllVersion, config.channelCount);
                    for (unsigned i = 0; i < config.channelCount; ++i)
                    {
                        auto& ch = config.channel[i];
                        std::fprintf(stderr, "[vector]   ch[%u]: name=\"%.31s\" idx=%u mask=0x%llX hwCh=%u sn=%u busType=0x%X\n", i, ch.name, ch.channelIndex, ch.channelMask,
                                     ch.hwChannel, ch.serialNumber, ch.connectedBusType);
                        if (ch.channelMask == 0)
                            continue;

                        device_descriptor d;
                        d.kind = adapter_kind::vector_xl;
                        d.port = std::format("xl:{}", ch.channelIndex);
                        d.friendly_name = std::format("Vector {} CH{} (S/N {})", ch.name, ch.hwChannel + 1, ch.serialNumber);
                        out.push_back(std::move(d));
                    }
                }
                else
            ...
            {
                std::fprintf(stderr, "[vector] vxlapi: driver not available (see errors above)\n");
            }
        }
#endif

#if !defined(JCAN_HAS_SOCKETCAN) && !defined(_WIN32) && (defined(JCAN_HAS_VECTOR) || defined(JCAN_HAS_KVASER))
        {
                        ...
                        if (auto* vdev = find_known_vector(pid); vdev)
                        {
                            ...
                            if (udesc.iProduct)
                            {
                                ...
                                if (libusb_open(devlist[i], &h) == 0 && h)
                                {
                                    ...
                                    int len = libusb_get_string_descriptor_ascii(h, udesc.iProduct, buf, sizeof(buf));
                                    if (len > 0)
                                        product.assign(reinterpret_cast<char*>(buf), static_cast<size_t>(len));
                                    libusb_close(h);
                                }
                            }
                            if (product.empty())
